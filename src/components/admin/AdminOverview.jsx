...
const AdminOverview = () => {
  ...
  const [viewMode, setViewMode] = useState('daily'); // 'daily' atau 'monthly'
    ...
    try {
      ...
      const groupedData = data.reduce((acc, order) => {
        const date = viewMode === 'daily'
          ...
          : new Date(order.created_at).toLocaleDateString('id-ID', { year: 'numeric', month: 'long' });
        acc[date] = (acc[date] || 0) + order.total_amount;
        return acc;
      }, {});

      const revenueData = Object.entries(groupedData).map(([date, amount]) => ({ date, amount }));
      setDailyRevenue(revenueData);

      // Menghitung total pendapatan berdasarkan data yang ditampilkan
      const totalRev = revenueData.reduce((sum, item) => sum + item.amount, 0);
  ...
  const revenueChartData = {
    ...
    datasets: [
      {
        label: viewMode === 'daily' ? 'Pendapatan Harian' : 'Pendapatan Bulanan',
